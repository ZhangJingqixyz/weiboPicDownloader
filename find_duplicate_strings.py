def find_duplicate_strings(input_string):
    ...
    for s in strings:
        # 去除可能的引号
        clean_s = s.strip('"\'')
        count_dict[clean_s] = count_dict.get(clean_s, 0) + 1
    
    # 找出重复的字符串
    duplicates = [s for s, count in count_dict.items() if count > 1]
